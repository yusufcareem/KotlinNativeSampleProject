...
public protocol ImmediateScheduler {
    func schedule<StateType>(state: StateType, com.careem.adma.booking.framework.action: (/*ImmediateScheduler,*/ StateType) -> RxResult<Disposable>) -> RxResult<Disposable>
}
```
...
public protocol Scheduler: ImmediateScheduler {
    ...
    func scheduleRelative<StateType>(state: StateType, dueTime: TimeInterval, com.careem.adma.booking.framework.action: (StateType) -> RxResult<Disposable>) -> RxResult<Disposable>
}
```
...
public protocol PeriodicScheduler : Scheduler {
    func schedulePeriodic<StateType>(state: StateType, startAfter: TimeInterval, period: TimeInterval, com.careem.adma.booking.framework.action: (StateType) -> StateType) -> RxResult<Disposable>
}
```
...
If `CurrentThreadScheduler.instance.schedule(state) { }` is called for the first time on some thread, the scheduled com.careem.adma.booking.framework.action will be executed immediately and a hidden queue will be created where all recursively scheduled actions will be temporarily enqueued.

If some parent frame on the call stack is already running `CurrentThreadScheduler.instance.schedule(state) { }`, the scheduled com.careem.adma.booking.framework.action will be enqueued and executed when the currently running com.careem.adma.booking.framework.action and all previously enqueued actions have finished executing.

## MainScheduler (Serial scheduler)

Abstracts work that needs to be performed on `MainThread`. In case `schedule` methods are called from main thread, it will perform the com.careem.adma.booking.framework.action immediately without scheduling.
