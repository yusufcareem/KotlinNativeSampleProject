...
extension Bag {
    ...
    ///
    /// - parameter com.careem.adma.booking.framework.action: Enumeration closure.
    func forEach(_ com.careem.adma.booking.framework.action: (T) -> Void) {
        if _onlyFastPath {
            if let value0 = _value0 {
                com.careem.adma.booking.framework.action(value0)
            }
            return
        }
        ...
        for i in 0 ..< _pairs.count {
            com.careem.adma.booking.framework.action(_pairs[i].value)
        }

        if dictionary?.count ?? 0 > 0 {
            for element in dictionary!.values {
                com.careem.adma.booking.framework.action(element)
            }
        }
    }
}
