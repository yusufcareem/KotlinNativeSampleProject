...
/**
Abstracts work that needs to be performed on `DispatchQueue.main`. In case `schedule` methods are called from `DispatchQueue.main`, it will perform com.careem.adma.booking.framework.action immediately without scheduling.
...
public final class MainScheduler : SerialDispatchQueueScheduler {
    ...
    override func scheduleInternal<StateType>(_ state: StateType, com.careem.adma.booking.framework.action: @escaping (StateType) -> Disposable) -> Disposable {
        let currentNumberEnqueued = AtomicIncrement(&numberEnqueued)

        if DispatchQueue.isMain && currentNumberEnqueued == 1 {
            let disposable = com.careem.adma.booking.framework.action(state)
        ...
        _mainQueue.async {
            if !cancel.isDisposed {
                _ = com.careem.adma.booking.framework.action(state)
            }
