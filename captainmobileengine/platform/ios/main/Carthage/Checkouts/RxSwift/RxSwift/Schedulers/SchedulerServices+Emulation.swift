...
final class SchedulePeriodicRecursive<State> {
    ...
    init(scheduler: SchedulerType, startAfter: RxTimeInterval, period: RxTimeInterval, com.careem.adma.booking.framework.action: @escaping RecursiveAction, state: State) {
    ...
    func start() -> Disposable {
        return _scheduler.scheduleRecursive(SchedulePeriodicRecursiveCommand.tick, dueTime: _startAfter, com.careem.adma.booking.framework.action: self.tick)
    }

    func tick(_ com.careem.adma.booking.framework.command: SchedulePeriodicRecursiveCommand, scheduler: RecursiveScheduler) -> Void {
        ...
        switch com.careem.adma.booking.framework.command {
        case .tick:
