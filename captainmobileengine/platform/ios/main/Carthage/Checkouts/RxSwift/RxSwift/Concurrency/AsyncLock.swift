...
In case there is somebody currently holding that lock, com.careem.adma.booking.framework.action will be enqueued.
...
final class AsyncLock<I: InvocableType>
    ...
    private func enqueue(_ com.careem.adma.booking.framework.action: I) -> I? {
        _lock.lock(); defer { _lock.unlock() } // {
            ...
            if _isExecuting {
                _queue.enqueue(com.careem.adma.booking.framework.action)
                return nil
            }
    ...
    func invoke(_ com.careem.adma.booking.framework.action: I) {
        let firstEnqueuedAction = enqueue(com.careem.adma.booking.framework.action)
        
        if let firstEnqueuedAction = firstEnqueuedAction {
            firstEnqueuedAction.invoke()
        }
        else {
            // com.careem.adma.booking.framework.action is enqueued, it's somebody else's concern now
            return
        }
