...
class DefaultWikipediaAPI: WikipediaAPI {
    ...
    // Example wikipedia response http://en.wikipedia.org/w/api.php?com.careem.adma.booking.framework.action=opensearch&search=Rx
    func getSearchResults(_ query: String) -> Observable<[WikipediaSearchResult]> {
        let escapedQuery = query.URLEscaped
        let urlContent = "http://en.wikipedia.org/w/api.php?com.careem.adma.booking.framework.action=opensearch&search=\(escapedQuery)"
    ...
    // http://en.wikipedia.org/w/api.php?com.careem.adma.booking.framework.action=parse&page=rx&format=json
    func articleContent(_ searchResult: WikipediaSearchResult) -> Observable<WikipediaPage> {
        let escapedPage = searchResult.title.URLEscaped
        guard let url = URL(string: "http://en.wikipedia.org/w/api.php?com.careem.adma.booking.framework.action=parse&page=\(escapedPage)&format=json") else {
